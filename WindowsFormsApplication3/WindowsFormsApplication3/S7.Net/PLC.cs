...
namespace PlcCommon.S7.Net
{
    ...
    public partial class Plc : IDisposable
    {
        ...
        public Plc(CpuType cpu, string ip, int port, Int16 rack, Int16 slot)
        {
            ...
            {
                Logger.E($"The value of argument '{nameof(cpu)}' ({cpu}) is invalid for Enum type '{typeof(CpuType).Name}'.");
            ...
            {
                Logger.E($"IP ({ip}) address must valid.");
        ...
        private void AssertPduSizeForRead(ICollection<DataItem> dataItems)
        {
            ...
            if ((dataItems.Count + 1) * 12 > MaxPDUSize)
            {
            ...
            {
                Logger.E($"Too much data requested for read");
                //throw new Exception("Too much data requested for read");
            }
        }
        ...
        private void AssertPduSizeForWrite(ICollection<DataItem> dataItems)
        {
            ...
            {
                Logger.E($"Too many vars supplied for write");
            ...
            {
                Logger.E($"Too much data supplied for write");
        ...
        private void ConfigureConnection()
        {
            ...
            tcpClient.ReceiveTimeout = (int)TimeSpan.FromSeconds(5).TotalMilliseconds;
            tcpClient.SendTimeout = (int)TimeSpan.FromSeconds(5).TotalMilliseconds;
        }
