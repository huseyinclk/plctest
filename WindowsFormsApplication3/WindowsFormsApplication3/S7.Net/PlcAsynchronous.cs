...
using PlcCommon.S7.Net.Protocol;
...
namespace PlcCommon.S7.Net
{
    ...
    public partial class Plc
    {
        ...
        public async Task OpenAsync()
        {
            ...
            {
                Logger.E($"Error reading Connection Confirm   if (response.PDUType({response.PDUType}) != 0xd0)");
            ...
            if (s7data == null)
            {
                Logger.E($"No data received in response to Communication Setup if (s7data({s7data}) == null)");
            ...
            if (s7data[1] != 0x03)
            {
                Logger.E($"Error reading Communication Setup response   if (s7data[1]({s7data[1]}) != 0x03)");
        ...
        public async Task<Tuple<int, object>> ReadClassAsync(object sourceClass, int db, int startByteAdr = 0)
        {
            ...
            if (numBytes <= 0)
            {
                Logger.E($"The size of the class is less than 1 byte and therefore cannot be read if (numBytes({numBytes}) <= 0)");
        ...
        public async Task<T> ReadClassAsync<T>(Func<T> classFactory, int db, int startByteAdr = 0) where T : class
        {
            ...
            if (readBytes <= 0)
            {
                Logger.E($" if (readBytes({readBytes}) <= 0)");
                return null;
            }
        ...
        public async Task WriteBitAsync(DataType dataType, int db, int startByteAdr, int bitAdr, bool value)
        {
            ...
            {
                Logger.E($"Addressing Error: You can only reference bitwise locations 0-7. Address {bitAdr} is invalid if (bitAdr < 0 || bitAdr > 7)");
        ...
        public async Task WriteBitAsync(DataType dataType, int db, int startByteAdr, int bitAdr, int value)
        {
            if (value < 0 || value > 1)
            {
                Logger.E($"Value must be 0 or 1 if (value({value}) < 0 || value > 1)");
        ...
        public async Task WriteAsync(DataType dataType, int db, int startByteAdr, object value, int bitAdr = -1)
        {
            if (bitAdr != -1)
            {
                ...
                else if (value is int)
                {
                    ...
                    {
                        Logger.E($"Addressing Error: You can only reference bitwise locations 0-7. Address {intValue} is invalid   if (intValue < 0 || intValue > 7)");
                ...
                {
                    Logger.E($"Value must be a bool or an int to write a bit {value}");
