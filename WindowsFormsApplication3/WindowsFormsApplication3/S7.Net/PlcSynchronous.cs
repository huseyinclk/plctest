...
namespace PlcCommon.S7.Net
{
    public partial class Plc
    {
        ...
        public bool Open()
        {
            try
            {
                if (Connect())
                {
                    ...
                    {
                        Logger.E($"Error reading Connection Confirm response({response}).PDUType != 0xd0");
                        return false;
                    ...
                    if (s7data == null)
                    {
                        Logger.E($"Error reading Connection Confirm if (s7data({s7data}) == null)");
                        return false;
                    ...
                    if (s7data[1] != 0x03)
                    {
                        Logger.E($"Error reading Connection Confirm if (s7data[1]({s7data}) != 0x03)");
                        return false;
            ...
            {
                Logger.E($"Couldn't establish the connection to {IP}.");
                Logger.E(exc);
        ...
        public void WriteBit(DataType dataType, int db, int startByteAdr, int bitAdr, bool value)
        {
            ...
            {
                Logger.E($"Addressing Error: You can only reference bitwise locations 0-7. Address {bitAdr} is invalid");
        ...
        public void Write(DataType dataType, int db, int startByteAdr, object value, int bitAdr = -1)
        {
            if (bitAdr != -1)
            {
                ...
                else if (value is int)
                {
                    ...
                    {
                        Logger.E($"Addressing Error: You can only reference bitwise locations 0-7. Address {bitAdr} is invalid  if (intValue({intValue}) < 0 || intValue > 7)");
