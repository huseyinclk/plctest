...
namespace PlcCommon.S7.Net.Protocol
{
    internal static class S7WriteMultiple
    {
        public static int CreateRequest(ByteArray message, DataItem[] dataItems)
        {
            ...
            foreach (var item in dataItems)
            {
                ...
                {
                    bool b = Convert.ToBoolean(item.Value);
                    if (item.BitAdr > 7)
                    {
                        Logger.E($"Cannot read bit with invalid {nameof(item.BitAdr)} '{item.BitAdr}'.");
                ...
                else
                {
                    ...
                    if ((len & 0xb1) == 1 && itemCount != dataItems.Length)
                    {
                        data.Add(0);
                    }
                }
        ...
        public static void ParseResponse(byte[] message, int length, DataItem[] dataItems)
        {
            if (length < 12)
            {
                Logger.E($"Not enough data received to parse write response.");
            ...
            if (messageError != 0)
            {
                Logger.E($"Write failed with error {messageError}.");
                //throw new Exception($"Write failed with error {messageError}.");
            }

            if (length < 14 + dataItems.Length)
            {
                Logger.E($"Not enough data received to parse individual item responses.");
            ...
            for (int i = 0; i < dataItems.Length; i++)
            {
                ...
                Logger.E($"Write of dataItem {dataItems[i]} failed with error code {result}.");
            }
