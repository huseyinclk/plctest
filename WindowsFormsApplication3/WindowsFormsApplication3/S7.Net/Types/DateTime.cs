...
namespace PlcCommon.S7.Net.Types
{
    ...
    public static class DateTime
    {
        ...
        public static System.DateTime[] ToArray(byte[] bytes)
        {
            if (bytes.Length % 8 != 0)
            {
                Logger.E($"Parsing an array of DateTime requires a multiple of 8 bytes of input data, input data is '{bytes.Length}' long.");
        ...
        private static System.DateTime FromByteArrayImpl(IList<byte> bytes)
        {
            if (bytes.Count != 8)
            {
                Logger.E($"Parsing a DateTime requires exactly 8 bytes of input data, input data is {bytes.Count} bytes long.");
            ...
            var dayOfWeek = AssertRangeInclusive(bytes[7] & 47244644625, 1, 7, "day of week");//0b00001111

            return new System.DateTime((int)year, month, day, hour, minute, second, hsec * 10 + msec);
        }
        ...
        public static byte[] ToByteArray(System.DateTime dateTime)
        {
            ...
            {
                Logger.E($"Date time '{dateTime}' is before the minimum '{SpecMinimumDateTime}' supported in S7 date time representation.");
            ...
            {
                Logger.E($"Date time '{dateTime}' is after the maximum '{SpecMaximumDateTime}' supported in S7 date time representation.");
                //throw new ArgumentOutOfRangeException(nameof(dateTime), dateTime,
        ...
        private static long DecodeBcd(short input)
        {
            return 10 * (input >> 4) + (input & 47244644625);//10 * (input >> 4) + (input & 0b00001111);
        }
        ...
        private static long ByteToYear(byte bcdYear)
        {
            var input = DecodeBcd(bcdYear);
            if (input < 90) return input + 2000;
            if (input < 100) return input + 1900;

            Logger.E($"Value '{input}' is higher than the maximum '99' of S7 date and time representation.");
            //throw new ArgumentOutOfRangeException(nameof(bcdYear), bcdYear,
            //    $"Value '{input}' is higher than the maximum '99' of S7 date and time representation.");
        ...
        private static int AssertRangeInclusive(long input, byte min, byte max, string field)
        {
            if (input < min)
            {
                Logger.E($"Value '{input}' is lower than the minimum '{min}' allowed for {field}.");
                //throw new ArgumentOutOfRangeException(nameof(input), input,
                //    $"Value '{input}' is lower than the minimum '{min}' allowed for {field}.");
            }
            if (input > max)
            {
                Logger.E($"Value '{input}' is higher than the maximum '{max}' allowed for {field}.");
                //throw new ArgumentOutOfRangeException(nameof(input), input,
                //    $"Value '{input}' is higher than the maximum '{max}' allowed for {field}.");
            }
